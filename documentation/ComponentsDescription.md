...
*   **Принцип Работы (Цикл):**
    ...
    2.  Для каждого полученного `BufferedChangeEvent`:
        *   **Проверка попыток:** Если количество попыток превысило `RetryCount`, пометить как ошибку и пропустить.
        *   **Фильтрация:** Если в конфигурации указан фильтр, применить `IFilter.IsMatch()`. Если `false` — пометить как успешно обработанное ("Filtered out") и пропустить.
        *   **Трансформация:** Если указан трансформер, применить `ITransformer.Transform()` и сохранить результат в `change.Data.TransformedData`.
        *   **Отправка:** Вызвать `IReceiver.SendAsync()`, передав финальные данные.
        *   **Обновление статуса:** Вызвать `_storage.UpdateChangeStatusAsync()` с результатом отправки (`Success` или `Failed`) и временем доставки.
    3.  Ждать фиксированный интервал `PollingIntervalMs`.
    4.  Повторить.
...
#### Компонент: `StateTransitionFilter`

*   **Назначение:** Реализация `IFilter` для фильтрации изменений на основе переходов состояний в конкретной колонке.
*   **Принцип Работы:**
    *   `IsMatch`: Проверяет, является ли тип изменения `Update`.
    *   Сравнивает старое и новое значение указанной колонки (`column`).
    *   Использует `StateTransitionChecker` для проверки допустимости перехода согласно выражению (`expression`).
    *   Кэширует скомпилированные правила переходов для повышения производительности.
